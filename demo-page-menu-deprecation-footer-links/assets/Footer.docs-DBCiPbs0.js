import{j as e}from"./jsx-runtime-z8MfsBtr.js";import{useMDXComponents as i}from"./index-BAvqEBP-.js";import{M as s,e as r,P as a,h as c}from"./index-BRu0fbPf.js";import{F as m,O as l}from"./Footer.stories-BvhZ2j7q.js";import"./index-C9rmetsa.js";import"./index-D7uoVdV3.js";import"./_commonjsHelpers-CqkleIqs.js";import"./iframe-D24NOPoY.js";import"./index-wgz3LQUQ.js";import"./index-Cu4lwwaE.js";import"./index-Brs1icLk.js";import"./index-ogSvIofg.js";import"./index.esm-BIWMn5vV.js";import"./clsx-B-dksMZM.js";import"./index.esm-U-3h8Od4.js";import"./Icon-BPwTIPQC.js";const p=`<!-- @license CC0-1.0 -->

# Footer

Provides service information at the bottom of every page.

## Guidelines

- The Footer usually consists of two sections: a full-width blue area and a [Page Menu](/docs/components-navigation-page-menu--docs) below it.
- A Footer containing only a Page Menu can be sufficient for applications.
- The Footer must be used on all websites for the City of Amsterdam.
- It must be the same on every page.

The top section offers space for various practical links:

- The first column focuses on contact information.
  The information is as specific as possible and tailored to the respective website or page (e.g., including a chat option if available).
- The second column contains links to relevant (online) sites or sources.
- The third column refers to newsletters, social media, etc.

The menu in the bottom section is for links to privacy policies, cookie statements, information about the website itself, etc.
Contact details should not go here.
`;/*@license CC0-1.0*/function o(n){const t={h2:"h2",h3:"h3",p:"p",...i(),...n.components};return e.jsxs(e.Fragment,{children:[`
`,`
`,e.jsx(s,{of:m}),`
`,e.jsx(r,{children:p}),`
`,e.jsx(a,{}),`
`,e.jsx(t.h2,{id:"examples",children:"Examples"}),`
`,e.jsx(t.h3,{id:"onderzoek-en-statistiek",children:"Onderzoek en Statistiek"}),`
`,e.jsx(t.p,{children:`The footer for a specific website can be a bit different.
Here’s an example for the ‘Onderzoek en Statistiek’ department.`}),`
`,e.jsx(c,{of:l})]})}function E(n={}){const{wrapper:t}={...i(),...n.components};return t?e.jsx(t,{...n,children:e.jsx(o,{...n})}):o(n)}export{E as default};
