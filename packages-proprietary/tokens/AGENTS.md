...
Where a value per side is wanted, split the token in two and set the longhands, as `ams.dialog.header.padding-block` was split into `ams.dialog.header.padding-block-start` and `ams.dialog.header.padding-block-end`.
