import { Canvas, Controls, Markdown, Meta, Primary } from "@storybook/blocks";
import * as IconStories from "./Icon.stories.tsx";
import README from "../../../../packages/css/src/components/icon/README.md?raw";

<Meta of={IconStories} />

<Markdown>{README}</Markdown>

# Usage

Use the React Icon component together with a React SVG component from `@amsterdam/design-system-react-icons`.
Import this SVG as follows:

```js
import { EmailIcon } from "@amsterdam/design-system-react-icons";
```

Then, you can use it in the component like this:

```js
<Icon svg={EmailIcon} />
```

<Primary />

<Controls />

## Examples

### With text

Icons can be used alongside text.
Using the same text levels for both the icon and text aligns them perfectly.
[Consult the documentation on typography for more information](/docs/docs-designrichtlijnen-typografie--docs#tekstniveaus).

<Canvas of={IconStories.WithText} />

### Square

Sometimes it’s useful for the icon to take up square space, for example, with the [Icon Button](https://amsterdam.github.io/design-system/?path=/docs/react_buttons-icon-button--docs).

<Canvas of={IconStories.Square} />

### Level 3

<Canvas of={IconStories.Level3} />

### Level 4

<Canvas of={IconStories.Level4} />

### Level 5

<Canvas of={IconStories.Level5} />

### Level 6

<Canvas of={IconStories.Level6} />
