...
import * as TableSortableHeaderCellStories from "./TableSortableHeaderCell.stories.tsx";
...
A Table is composed of a Caption, a Header containing Header Cells or Sortable Header Cells, a Body, an optional Footer, and Rows of Cells.
...
Labels a column and links to the table sorted by it.
Use a link rather than a button, so the sort order lives in the URL and the table can be bookmarked and shared.

Pass the direction the column is sorted in now to `sortDirection`, and the URL for the direction the link applies to `href`.
The column the table is sorted by is underlined; the chevron of the other columns recedes.

See [Table Page](/docs/pages-internal-table-page--docs) for the full pattern, including how to derive each `href`.

<Canvas of={TableSortableHeaderCellStories.SortableHeaderCell} />

<Controls of={TableSortableHeaderCellStories.SortableHeaderCell} />
