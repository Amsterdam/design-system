...
This field helps the user enter a whole number, we recommend using the `text` type for whole numbers.
You then need to add the `inputMode="numeric"` attribute and a pattern to validate the input.
Do not rely only on the browser’s validation. The `text` input field looks like a standard text input.
On some devices, it may show a numeric keypad to aid in entering whole numbers.
