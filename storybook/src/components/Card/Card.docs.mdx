...
Its `aspectRatio` sets the shape of the image itself.
...
Set `objectFit` to `contain` to keep the full source in view instead of the default cover fit.
...
A source whose proportions differ from the shape the Card gives its image fills that shape by default, losing whatever falls outside it.
Set `objectFit` to `contain` to keep all of it in view instead.
...
A Card gives its image a default aspect ratio of 16 by 9 through a Card-specific token.
...
Use `objectFit="contain"` when keeping all of the source in view is the intended visual choice.
