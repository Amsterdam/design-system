...
A Card draws its image in an area of a fixed shape instead of letting the image decide its own height.
...
Override `--ams-card-image-aspect-ratio` to give the area another shape.
