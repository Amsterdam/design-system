...
It accepts all props of a regular [Image](/docs/components-media-image--docs), such as a set of image sources.
Its `aspectRatio` gives the area the image is displayed in, rather than the image itself.
Give every Card in the same set the same value, so they all end up the same size.
Set `objectFit` to `cover` to have the image fill that area and be cropped to it, instead of fitting inside it.
...
An image whose aspect ratio differs from the area a Card gives it fits inside that area by default, so all of it stays in view.
Set `objectFit` to `cover` to fill the area instead, which crops the image.
Give the Cards in one set the same treatment, so the set reads as a whole.

<Canvas of={CardStories.ImageFit} />
...
A Card gives its image an area of a fixed shape instead of letting the image decide its own height.
...
The image fits inside that area by default and keeps all of it in view, against the same background colour it shows while it loads.
Cropping an image is a decision, so it is asked for rather than assumed.
A set of Cards whose images should reach the edges is what `objectFit` is for.
