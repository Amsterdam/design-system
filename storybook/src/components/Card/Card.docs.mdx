...
Set `objectFit` to `contain` to keep the full image in view instead of the default cover fit.
...
An image whose proportions differ from the shape the Card gives its image fills that shape by default, losing whatever falls outside it.
...
Use `objectFit="contain"` when keeping all of the image in view is the intended visual choice.
