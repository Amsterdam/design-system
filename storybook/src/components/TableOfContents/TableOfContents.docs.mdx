...
Set `collapsed={false}` on `TableOfContents.List` to open nested items by default.
This setting is inherited by nested lists unless they set their own `collapsed` value.
