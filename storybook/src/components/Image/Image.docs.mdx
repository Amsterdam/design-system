...
Leave `imageFit` to the default of `cover` unless important objects in the image dissapear or the cropping just looks weird.
...
Setting `imageFit` to `contain` keeps the whole source in view inside that same area instead.
