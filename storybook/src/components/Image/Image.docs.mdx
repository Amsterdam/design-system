...
A description is unnecessary for decorative images; use `alt=""` for those.
...
Additionally, it can be helpful for search engine optimisation.

## Content

Describe the content of the image if the image is not only decorative.
When choosing a description, imagine what you would say when reading the page to someone over the phone without mentioning that there is an image on the page (source: [MDN](https://developer.mozilla.org/en-US/docs/Web/API/HTMLImageElement/alt)).
