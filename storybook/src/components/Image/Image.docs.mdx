...
Leave `fit` unset to keep the default of `cover` unless important objects in the image disappear or the cropping does not look right.
Setting `fit` to `contain` keeps the whole image in view at the cost of empty space beside or above it.
...
Setting `fit` to `contain` ensures that the entire image is visible within its container, even if it means leaving empty space around it. This is useful when you want to avoid cropping important parts of the image.

<Canvas of={ImageStories.FittedImages} />
...
Setting `fit` to `contain` keeps the whole image in view inside that same area instead.
