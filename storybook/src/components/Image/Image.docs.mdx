...
Leave `objectFit` to the default of `cover` unless important objects in the image disappear or the cropping does not look right.
Setting it to `contain` keeps the whole image in view at the cost of empty space beside or above it.
...
Setting `objectFit` to `contain` keeps the whole source in view inside that same area instead.
