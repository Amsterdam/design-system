...
After all, the scarcer amounts of white space can’t fulfill that role.

Cells can be made transparent, e.g. the one that contains the Breadcrumb Heading.
This removes the padding as well.

The ‘flush’ appearance remove only the padding, keeping the white background.
This is useful for components that want to sit edge-to-edge inside the cell, like a Table.
