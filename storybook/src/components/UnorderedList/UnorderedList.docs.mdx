...
Therefore, this variant does not define typography for the items and the `color` prop to `inverse` has no effect.
...
Set the `color` prop to `inverse` if the List sits on a dark background.
