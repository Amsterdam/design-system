...
import tokens from "#tokens/components/ams/description-list.tokens.json";
...
The Term, Description, and Section subcomponents make up a Description List.
Term and Description render the `<dt>` and `<dd>` elements, and a Section groups a term with its descriptions.
...
Use a Description List for pairs of terms and descriptions, such as a glossary.

### When not to use

Use a [Table](/docs/components-containers-table--docs) to compare multiple items by the same properties.
Use an [Unordered List](/docs/components-text-unordered-list--docs) or [Ordered List](/docs/components-text-ordered-list--docs) for items without a term.

### How to use

Use [Margin](/docs/utilities-css-margin--docs) or [Prose](/docs/utilities-css-prose--docs) to add vertical whitespace.

Keep terms short so they fit in the column next to the descriptions.
...
- [Data List](/docs/components-containers-data-list--docs) – pairs labels with values instead of terms with meanings, and separates them by a line.
