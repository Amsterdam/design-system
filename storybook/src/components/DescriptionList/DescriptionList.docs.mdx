...
The Description List uses container queries to switch from a vertical to a horizontal orientation when a parent container is wider than a predefined breakpoint (see Design).

**Note**: the blue background on the Grid Cells is for demonstration purposes only.
