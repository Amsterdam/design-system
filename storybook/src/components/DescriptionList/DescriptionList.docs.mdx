...
The Description List uses container queries to switch from a vertical to a horizontal orientation when its nearest [query container](/docs/utilities-css-query-container--docs) is wider than a predefined breakpoint (see Design). If no query container is available, it displays in the default, vertical orientation.
