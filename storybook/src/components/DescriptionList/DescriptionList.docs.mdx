...
The Description List uses container queries to switch from a vertical to a horizontal orientation when a parent container is wider than a predefined breakpoint (see Design). If no container query context is available, it displays in the default, vertical orientation.
