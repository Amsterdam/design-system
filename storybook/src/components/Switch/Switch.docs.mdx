...
A Switch therefore takes its name from a [Label](/docs/components-forms-label--docs) or an `aria-label` you provide, which is what ‘How to use’ asks for; without one it is announced as an unnamed switch.
