...
A Link List renders `ul` and `li` elements with the link inside each item, so screen readers announce it as a list and report how many links it holds before reading them.

The chevron sits inside the link and adds nothing to its name.
It also belongs to the target, so the whole of it responds to a click, not only the words.

Because the chevron marks each link, colour is not the only thing distinguishing them from the text around them, as [WCAG 1.4.1](https://www.w3.org/TR/WCAG22/#use-of-color) requires.
