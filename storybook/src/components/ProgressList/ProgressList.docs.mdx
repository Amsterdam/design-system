...
This lets a parent component decide which steps are expanded — for example, to show only one step at a time.

<Canvas of={ProgressListStepStories.Controlled} />

When you omit `collapsed`, the step manages its own state using `defaultCollapsed`.
