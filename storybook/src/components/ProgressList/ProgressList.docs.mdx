...
The status indicators are visual only.
Use `completedAccessibleText` and `currentAccessibleText` to provide screen reader labels — the defaults are 'Klaar' and 'Bezig'.
