...
Until a component exists for it, the Card tagline carries those values comma separated: here the category and the district, which are the two facets the filter column offers.

Close the Card with when and where it happens, below the description, as one small Paragraph: the date, the time, and then the venue after a middot.
Those two answer a different question from the facets above — not what kind of activity this is, but whether the visitor can be there — so they read better as a footnote to the description than as a second tagline.
A middot separates the venue from the date without implying the sentence continues, which a comma would.
