...
The landing page can list the steps instead, and [With a progress indicator](#with-a-progress-indicator) says when a counter still helps.
...
Form pages usually don’t have their own step counter for good reasons:

- A branching form cannot show a fixed total, since the count changes based on which path users take.
- A counter takes up space above the question and reminds users how many steps remain, which may discourage them.
- The landing page’s step list already sets expectations once, when users decide to begin.

Start by testing the form without a counter.
...
This deviates from the [NL Design System’s own step counter guidance](https://nldesignsystem.nl/richtlijnen/formulieren/meerdere-stappen/voortgang-tonen/), which also puts the count in the page `<title>`; this pattern only shows it above the heading.
