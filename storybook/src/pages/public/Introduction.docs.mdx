...
### The Grid with the content header

The Grid that follows the Breadcrumb holds the page title, usually with an optional metadata paragraph and a lead paragraph.
It has no `paddingTop`, so that the breadcrumb and the page title read as a single block.
For the space between those three, see [vertical space](/docs/docs-designer-guide-vertical-space--docs).

Its `paddingBottom` depends on the section below it.
A section on a white background takes `2x-large`, which sets the header apart from the body that follows.
A coloured section takes `x-large`, because the edge of the colour marks that boundary already.
A Spotlight and an Image are the two that occur in these templates.

Not every page keeps its header in a Grid of its own.
Where the header shares a Grid with the content below it, the row gap sets the space between the two.
...
See [the Grid with the content header](#the-grid-with-the-content-header) for the space around and inside this section.
