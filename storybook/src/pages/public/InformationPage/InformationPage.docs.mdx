...
- Where the lead paragraph takes a Cell of its own, the Grid’s row gap sets the space between it and the metadata above.
  That gap stays at `x-large` rather than the medium that [vertical space](/docs/docs-designer-guide-vertical-space--docs) asks for within a Cell: the regular background of the page runs along it — at the left-hand side, and across the full width on the narrow grid — so the wider gap still reads as one Content Header.
...
The lead paragraph and an introductory Image sit side by side below the title, and stack on the narrow grid.
