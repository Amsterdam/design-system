...
By querying only on viewport width, and by not specifying a [media type](https://developer.mozilla.org/en-US/docs/Web/CSS/Reference/At-rules/@media#media_types), the media type `all` is implied and the page layout will also look correct when printing.
