...
import { AnchorMdx, Meta } from "@storybook/addon-docs/blocks";
...
By only querying on viewport width and [not querying on media type](https://developer.mozilla.org/en-US/docs/Web/CSS/Reference/At-rules/@media#media_types) `all` is implied and the page layout will look correct when printing.
