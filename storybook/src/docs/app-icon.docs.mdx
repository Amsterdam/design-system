...
Provide a full app name and optionaklly a short app name if you want to generate a Web Manifest as well. Users will may then be prompted to install the app as a PWA:
...
> If an `index.html` is found in the root or in `public` the script will also insert the links in the HTML's `<head>` tag. This will not be the case for default Next.js applications.
...
The symlinks and copied and edited files can be commited to the repository.
