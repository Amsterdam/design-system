import { Meta, Typeset } from "@storybook/blocks";

<Meta title="Docs/Design Guidelines/Typography" />

# Typography

## Text Size

### Seven Text Levels

There are 7 levels of text size and corresponding line height.
Each typographic element belongs to one of these levels: title, paragraph, link, quote, form text, etc.
They are numbered from 0 to 6 to align them with the levels of headings.
Examples:

| Level | Components                             |
| ----: | :------------------------------------- |
|     0 | Page heading                           |
|     1 | Heading 1                              |
|     2 | Heading 2                              |
|     3 | Heading 3, Blockquote                  |
|     4 | Heading 4, Large Paragraph             |
|     5 | Paragraph, List, Link, Button          |
|     6 | Small Paragraph, Page Menu, Breadcrumb |

### Responsive

The text size depends on the window width, ensuring pleasant reading on screens of all sizes.
This also complements the responsive [grid](?path=/docs/react_layout-grid--docs).

Each level has a minimum text size, active up to a window width of 320 pixels.
There’s also a maximum text size from a window width of 1600 pixels.
Between these points, the text grows linearly.

### Typographic Scale

The typographic scale is based on a factor of 5 ÷ 4 = 1.25.
For each adjacent level, the text is that much larger or smaller.

The text size grows more slowly at the minimum window width, with a factor of 7 ÷ 6 ≈ 1.17.
This prevents titles from becoming too large on phones and other narrow windows and small texts from becoming too small.

### Two Themes for Websites and Applications

#### Spacious

The typography is quite spacious for websites, in line with the overall design.
A standard paragraph is 18 pixels at a window width of 320 pixels.
It grows to 24 pixels up to a width of 1600 pixels.

The maximum text sizes for all levels in the spacious theme:

<Typeset
  fontFamily="Amsterdam Sans, Arial, sans-serif"
  fontSizes={[73.2, 58.6, 46.9, 37.5, 30, 24, 19.2]}
  fontWeight={400}
  sampleText="Jouw typograaf biedt mij zulke exquise schreven"
/>

And the minimum:

<Typeset
  fontFamily="Amsterdam Sans, Arial, sans-serif"
  fontSizes={[38.9, 33.3, 28.6, 24.5, 21, 18, 15.4]}
  fontWeight={400}
  sampleText="Jouw typograaf biedt mij zulke exquise schreven"
/>

#### Compact

For applications, such large text is not necessary, even counterproductive.
Hence, there’s a compact theme for typography.
In this theme, a paragraph is 14 pixels at the minimum window width and 18 pixels at the maximum.
Text is a quarter smaller and grows slightly more slowly.

The maximum text sizes for all levels in the compact theme:

<Typeset
  fontFamily="Amsterdam Sans, Arial, sans-serif"
  fontSizes={[54.9, 43.9, 35.2, 28.1, 22.5, 18, 14.4]}
  fontWeight={400}
  sampleText="Jouw typograaf biedt mij zulke exquise schreven"
/>

And the minimum:

<Typeset
  fontFamily="Amsterdam Sans, Arial, sans-serif"
  fontSizes={[30.3, 25.9, 22.2, 19.1, 16.3, 14, 12]}
  fontWeight={400}
  sampleText="Jouw typograaf biedt mij zulke exquise schreven"
/>

### Window Widths for Design

In practice, a window can have any width.
Designing for all those widths is impractical.
Therefore, we work with three reference widths in Figma: 320, 832, and 1600 pixels.
At those sizes, we develop a screen for those three classes of devices.
The text sizes at those points are as follows, rounded to 1 decimal place:

#### Spacious

| Device Class | Design Width |    0 |    1 |    2 |    3 |    4 |    5 |    6 |
| :----------- | ------------ | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| Phone        | 320          | 38.9 | 33.3 | 28.6 | 24.5 |   21 |   18 | 15.4 |
| Tablet       | 832          | 52.6 | 43.4 | 35.9 | 29.7 | 24.6 | 20.4 | 16.9 |
| Desktop      | 1600         | 73.2 | 58.6 | 46.9 | 37.5 |   30 |   24 | 19.2 |

#### Compact

| Device Class | Design Width |    0 |    1 |    2 |    3 |    4 |    5 |    6 |
| :----------- | ------------ | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| Phone        | 320          | 30.3 | 25.9 | 22.2 | 19.1 | 16.3 |   14 |   12 |
| Tablet       | 832          | 40.1 | 33.1 | 27.4 | 22.7 | 18.8 | 15.6 |   13 |
| Desktop      | 1600         | 54.9 | 43.9 | 35.2 | 28.1 | 22.5 |   18 | 14.4 |

## Emphasis

There are various ways to emphasize text.
Here are our choices.

### Weight

We use 2 weights online: regular and extra bold.
In CSS terms: 400 and 800.

<Typeset
  fontFamily="Amsterdam Sans, Arial, sans-serif"
  fontSizes={[24]}
  fontWeight={400}
  sampleText="Jouw typograaf biedt mij zulke exquise schreven"
/>

Text is bolded in titles and decorative quotes.

<Typeset
  fontFamily="Amsterdam Sans, Arial, sans-serif"
  fontSizes={[24]}
  fontWeight={800}
  sampleText="Jouw typograaf biedt mij zulke exquise schreven"
/>

### Style

Avoid italicized text as much as possible, as it’s not easily readable.
Use it only to format a fragment, not for entire blocks of text.
There are better ways to emphasize long texts.

<Typeset
  fontFamily="Amsterdam Sans, Arial, sans-serif"
  fontSizes={[24]}
  sampleText="Jouw typograaf biedt mij zulke exquise schreven"
  style={{ fontStyle: "italic" }}
/>

### Underlining

Avoid underlining text.
Underline only interactive links.

The line thickness is 2 pixels.
The distance between the underline and the baseline is 6 pixels –
more than we can show in the example below.
When the mouse hovers over the link, the line at the top becomes one pixel thicker.

<Typeset
  fontFamily="Amsterdam Sans, Arial, sans-serif"
  fontSizes={[24]}
  lineHeight={1.6}
  sampleText="Jouw typograaf biedt mij zulke exquise schreven"
  style={{ color: "#004469", textDecorationLine: "underline", textDecorationThickness: 2, textUnderlineOffset: 4 }}
/>

### Overhang

The space between letters (kerning) has been optimized in the typeface.
Adjusting kerning is not allowed.

## Other Properties

### Contrast

The contrast between text and background colour must meet the latest legal accessibility requirements of [WCAG, level AA](https://www.w3.org/WAI/WCAG21/Understanding/contrast-minimum.html).

Most text in the house style is pure black on pure white, meeting the requirement.
We make text white on most dark backgrounds, providing sufficient contrast.
For some background colours, black text is also readable.
Consult the respective components for specific guidelines.

### Alignment

Text is almost always left-aligned.

Exceptions:

1. The label of a button is centred.
2. In a table, it might be better to centre or right-align text.

### Line Length

Long lines of text are challenging to read.
Around 80 characters per line is the maximum.
In the 12-column [grid](?path=/docs/react_layout-grid--docs), paragraphs meet this when they occupy 8 columns or fewer.

### Word Breaking

The Dutch language has many long words, such as ‘gemeenteraadsverkiezingen’ (municipal elections).
Ensure in your application that these words are hyphenated neatly and do not break out of their container or get hidden.
One way to do this is with the following CSS snippet, combining `hyphens` and `word-break`:

```css
.class {
  hyphens: auto;
  /* only hyphenate words longer than 12 chars, for browsers that support it */
  hyphenate-limit-chars: 12;
  /* break overflowing words for browsers that don’t support CSS hyphens */
  overflow-wrap: break-word;
}
```

Further reading: [Wrapping and breaking text op MDN](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_text/Wrapping_breaking_text)
