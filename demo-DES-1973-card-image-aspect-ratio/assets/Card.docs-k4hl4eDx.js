...
It accepts all props of a regular `,(0,M.jsx)(r.a,{href:`/docs/components-media-image--docs`,children:`Image`}),`, such as a set of image sources.
Its `,(0,M.jsx)(r.code,{children:`aspectRatio`}),` gives the area the image is displayed in, rather than the image itself.
Give every Card in the same set the same value, so they all end up the same size.
Set `,(0,M.jsx)(r.code,{children:`objectFit`}),` to `,(0,M.jsx)(r.code,{children:`cover`}),` to have the image fill that area and be cropped to it, instead of fitting inside it.
...
`,(0,M.jsxs)(r.p,{children:[`Contains the name and link of the Card.
...
`,(0,M.jsx)(r.p,{children:`Give a Card one link and no more.
...
`,(0,M.jsx)(a,{of:h}),`
`,(0,M.jsx)(r.h3,{id:`fitting-the-image`,children:`Fitting the image`}),`
`,(0,M.jsxs)(r.p,{children:[`An image whose aspect ratio differs from the area a Card gives it fits inside that area by default, so all of it stays in view.
Set `,(0,M.jsx)(r.code,{children:`objectFit`}),` to `,(0,M.jsx)(r.code,{children:`cover`}),` to fill the area instead, which zooms and crops the image.
Give the Cards in one set the same treatment, so the set reads as a whole.`]}),`
`,(0,M.jsx)(a,{of:_}),`
...
`,(0,M.jsx)(r.p,{children:`The mandatory title of a Card is a link within a Heading.
...
`,(0,M.jsx)(r.p,{children:`The whole Card is made clickable by stretching an invisible layer from the link across it.
...
`,(0,M.jsx)(r.p,{children:`The focus ring is drawn around the entire Card instead of around the title alone, and the link’s own ring is switched off.
...
`,(0,M.jsx)(r.p,{children:`A Card gives its image an area of a fixed shape instead of letting the image decide its own height.
An image in a portrait aspect ratio is otherwise taller than the Card is wide, which leaves the text of one Card in a grid far below that of the next.
The area is 16 by 9 to begin with, the same shape an Image has, so a Card that is left alone needs no thought about it.`}),`
`,(0,M.jsx)(r.p,{children:`The image fits inside that area by default and keeps all of it in view, against the same background colour it shows while it loads.
Cropping an image is an artistic decision, so it requires a conscious choice.`}),`
