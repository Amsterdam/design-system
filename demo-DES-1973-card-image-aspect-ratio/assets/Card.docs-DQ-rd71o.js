...
A Card caps the height of the image, so a portrait aspect ratio is cropped rather than drawn at full height.
...
`,(0,j.jsxs)(r.p,{children:[`A Card draws its image in an area of a fixed shape instead of letting the image decide its own height.
...
An image taller than that area is cropped to it, and one that is shorter is centred in it, against the same background colour it shows while it loads.
Override `,(0,j.jsx)(r.code,{children:`--ams-card-image-aspect-ratio`}),` to give the area another shape.`]}),`
